async function chart() {
  ...
  var category = ["Global", "Northern", "Southern"];
  ...
  d3.csv("../data/Annual_By_Hemisphere.csv", (data) => {
    var res = data.map((d, i) => {
      return {
        date: parseDate(d.Year),
        season: d.Hemisphere,
        temperature: +d.AnnualAvg,
      };
    });
    ...
    renderChart(); // inital chart render (set default to Global)
    ...
    function renderChart() {
      ...
      glines
        ...
        .style("stroke", (d, i) => {
          return color(d.values[0].season)
        })
        .style("fill", "none")
